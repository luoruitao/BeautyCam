...
void BeautyCam::initMainImgUI()
{
	...
	m_MainImg = frame;
	//m_MainImg = imread("33.jpg");

	namedWindow("BeautyCam", WINDOW_AUTOSIZE);
	//string path = "33.jpg";
	//m_MainImg = imread(path);
	imshow("formal", m_MainImg);

	//检测人脸数据68点
	m_vecFaceData = dectectFace68("image.jpg");
...
std::vector<std::vector<Point2f>> BeautyCam::dectectFace68(const string& path)
{
	...
	for (auto var : dets)
	{
		//关键点检测器
		shape_predictor sp;
		deserialize("D:\\BaiduSyncdisk\\coding\\Project4\\Project4\\shape_predictor_68_face_landmarks.dat") >> sp;
